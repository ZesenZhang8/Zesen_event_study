...
####checking the COLUMS and COLWIDTHS:
print(COLWIDTHS)
print(COLUMNS)
    ...
    with open(pth, 'r') as file:
        for line in file:
            ...
            if line:
                ...
                ticker = ticker.strip().strip('"').lower()
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    _dat_file = os.path.join(DATDIR, f"{tic.lower()}_prc.dat")
    lines = []
    with open(_dat_file, 'r') as file:
        for line in file:
            lines.append(line.rstrip('\n'))
    return lines

###checking the function using 'aal_prc.dat' under the 'data' folder in project1:
print(read_dat('aal'))
...
def line_to_dict(line):
    ...
    for col in COLUMNS:
        width = COLWIDTHS[col]
        data_dict[col] = line[start:start+width]
        start += width
    return data_dict

###checking the function using "aal_prc.dat" first line of data to run and test the function:
test_line_aal_prc= '00000000000000 2005-09-27018.19491004943847719.299999221.0490021.399999618530273'
print(line_to_dict(test_line_aal_prc))
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst == 0:
        raise Exception("tickers_lst cannot be empty.")
...
def verify_cols(col_lst=None):
    ...
    if col_lst == 0:
        raise Exception("col_lst cannot be empty.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for tic in tickers_lst:
        ...
        data_dict[tic.lower()] = {'exchange': exchange, 'data': []}

        lines = read_dat(tic.lower())
        for line in lines:
            data_test = line_to_dict(line)
            col_test = {col: data_test[col] for col in col_lst if col in data_test}
            data_dict[tic.lower()]['data'].append(col_test)
...
if __name__ == "__main__":
    ...
    output_file = os.path.join(ROOTDIR, 'output.json')
    create_json(data_dict, output_file)

    # ----------------#    Please put your answers for the last question here:
''''
To answer the opening questions, following by number 1-3.
    
    1, Explain why to configure as in step 1 (5 marks).
    
    Answer: Setting up paths and directories, by using 'import os' and 'import toolkit_config', to access the 'project1'
    and necessary files. The ROOTDIR is to access 'project1' file by the complete path in computer. (For example:
    /User/homepage/PycharmProjects/toolkit/project1) The TICPATH and DATDIR are accessing the different files inside
    the 'project1' file. DATDIR is accessing the 'data' folder inside 'project1' file that is the reason why using
    ROOTDIR configuring path. TICPATH is accessing the 'TICKERS.txt' file inside 'project1' file. 
    
    2, From step 9, we can obtain a JSON file that contains ticker names, listed exchanges, trading volume,
    ...
    Answer: I think the first hypothesis is more likely to be true because the data is containing the ticker names,
    listed exchanges, trading volume, stock prices, and compute stock return from stock prices, which all are the 
    existing values and information. The existing values and information can be use to evaluate the firms' status. 
    For example, the negative words and decreasing stock returns are mostly likely describing the firms' current and 
    short-term predicted status. However, these information is not beyond firm fundamentals since there no updated
    information to indicate the uncover information. 
    In order to satisfy the second hypothesis, the journalist will be more likely evaluate and point out the risks, 
    management problems, and long-term predicted values,etc, instead of the existing fundamental information.
    And these implications are beyond the firm fundamentals and using to predict or reveal the actual circumstances 
    of the firms. 
    
    3, Given the hypothesis you have chosen, evaluate the short-run predictability for the trading volume
    with reason(s). (5 marks)
    
    Answer: Given the hypothesis of journalist reflecting the investor's evaluation of firms, there are some
    negative words and decreasing stock prices prediction in the article. Investors are more likely trade more 
    frequently to avoid risks and losses, therefore, the trading volume is expecting to increase in the short-run. 
    Also,for the potential stock with decreasing prices, investor might think as opportunities to invest and this 
    might also lead to the increase in trading volume. 
